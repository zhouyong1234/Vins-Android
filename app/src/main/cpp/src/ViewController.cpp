...
//        __android_log_print(ANDROID_LOG_DEBUG, "IMU", "IMU header: %f", imu_msg_buf.front()->header);

        if (imu_msg_buf.empty() || img_msg_buf.empty())
            return measurements;

        __android_log_print(ANDROID_LOG_DEBUG, "IMU", "imu header: %lf", imu_msg_buf.back()->header);
        __android_log_print(ANDROID_LOG_DEBUG, "IMG", "img header: %lf", img_msg_buf.front()->header);
